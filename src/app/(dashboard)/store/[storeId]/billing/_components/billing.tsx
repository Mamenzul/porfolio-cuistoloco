...
export async function Billing({
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Plan et utilisation</CardTitle>
          <div className="text-sm text-muted-foreground">
            Vous bénéficiez actuellement du plan{" "}
            <Badge
              variant="secondary"
            ...
            {plan?.isCanceled
              ? "Votre plan sera annulé le "
              : "Votre plan se renouvelle le "}
        ...
        <CardContent className="grid gap-6 sm:grid-cols-2">
          <UsageCard
            ...
            limit={productLimit}
            moreInfo="Le nombre de produits que vous pouvez créer avec votre plan actuel"
          />
        </CardContent>
      </Card>
            ...
            <CardFooter className="pt-4">
              {item.title === "Gratuit" ? (
                <Button className="w-full" asChild>
                  <Link href="/dashboard">
                    Choisir ce plan
                    <span className="sr-only">Choisir ce plan</span>
                  </Link>
                </Button>
              ) : (
                <ManagePlanForm
