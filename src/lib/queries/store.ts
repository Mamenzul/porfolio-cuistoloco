...
import { orders, products, stores, type Store } from "@/server/db/schema";
...
export async function getFeaturedStores() {
  ...
  try {
    const res = db
      ...
      .where(not(isNull(stores.stripeAccountId)))
      .groupBy(stores.id)
...
export async function getStoreByUserIdSlug(input: { userId: string }) {
  noStore();
  try {
